...
export default Todo;
/*class Todo {
  constructor(data, selector, handleCheck, handleDelete, handleTotalChange) {
    this._data = data;
    this._completed = data.completed || false;
    this._name = data.name;
    this._id = data.id;
  ...
  _setEventListeners() {
    ...
    this._todoDeleteBtn.addEventListener("click", () => {
      ...
      if (this._handleTotalChange) {
        this._handleTotalChange(-1); // always decrement total count on delete
      }
      this._todoElement.remove();
      console.log(`Deleted: ${this._data.name}`);
    });
